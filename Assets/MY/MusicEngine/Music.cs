...
public class Music : MonoBehaviour
{
	...
	public class Section
	{
		...
		public Section(int startBar, int mtBeat = 4, int mtBar = 16, double tempo =200)
		{
			StartBar = startBar;
			UnitPerBeat = mtBeat;
			UnitPerBar = mtBar;
		...
		public void OnValidate(int startTimeSample)
		{
			...
			// Debug.Log("クラスの方のOnvalidate、starttiemsampleが"+StartTimeSamples);
		}
	...
	//---------------------再生中の(current)のプロパティのゲッターやセッターのエリア-----------------------------
	...
	//メソッドのように使用することは出来ない。 music.UnitperBar = みたいに使う
	...
	//～～～～～～～～～～～AudioSourceのメソッドの改造品エリア～～～～～～～～～～～～～～～～～～～
	#region public static functions
	/// <summary>
	/// Change Current Music.
	/// </summary>
	/// <param name="MusicName">name of the GameObject that include Music</param>
	public void Play(string musicName, string sectionName = "") { MusicList_.Find((Music m) => m.name == musicName).PlayStart(sectionName); }//staticじゃなくしてます
	/// <summary>
	/// Quantize to musical time.
	/// </summary>
	public static void QuantizePlay(AudioSource source, int transpose = 0, float allowRange = 0.3f)//PDF19ページに詳細が記載
	{
	...
	public static void Seek(Timing timing)
	{
		...
		Current_.musicSource_.timeSamples = section.StartTimeSamples + (int)(deltaMT* MusicTimeUnit * Current_.samplingRate_);//スタートタイムサンプルが始動
		//Music.ゲットコンポーネントしたAS.曲が始まってからの経過 =  今のセクション.
	}
	...
	{
		// Debug.Log("<color=red>SetNextSection(インデックス)が実行</color>");
	...
	{
		// Debug.Log("<color=red>SetNextSection(名前)が実行</color>");
		SetNextSection(Current_.Sections.FindIndex((Section s) => s.Name == name), syncType);
	}
	#endregion
	//ここまでAudioSourceの改造品エリア

	//privateなパラメータ。だいたいメソッドとかゲッターで呼び出す
	...
	public void OnClickAwake()//カレントに移動と初期化を行っている
	{
		Debug.Log("<color=red>OnClicAwakeが実行</color>");
#if UNITY_EDITOR
		// if( !UnityEditor.EditorApplication.isPlaying )
		// // if(flag)//バグります。上は最初にdevide0が出ても大丈夫
		// {
		// 	Debug.Log("isPlaying がtrue に なってて onvalidateが起動");
		// 	OnValidate();
		// 	return;
		// }
#endif
		MusicList_.Add(this);//81行目 Musicというクラスのリスト
		// Debug.Log("MusicLise_の数は:"+MusicList_.Count);//再生時は1だった。Sectionの方が大事だからあんまり関係ない
		musicSource_ = GetComponent<AudioSource>();
		if( Current_ == null || musicSource_.playOnAwake )
		{
			// Debug.Log("Current_を変更");
			Current_ = this;//既に鳴ってるものがあれば、それをCurrentにする
		}
		samplingRate_ = musicSource_.clip.frequency;//サンプリングレートをAudioClipのサンプリングレートと合わせている
		...
		// {
		// 	Debug.Log("<color=red>セクション作成？</color>");
		...
		// 		clips[i] = AudioClip.Create(Sections[i].Name + "_clip", nextSectionSample - previousSectionSample, musicSource_.clip.channels, musicSource_.clip.frequency, false);//名前、与えられたサンプルの長さ、チャンネル、周波数からユーザーが AudioClip を作成します。
		// 		previousSectionSample = nextSectionSample;
		// 		float[] waveData = new float[clips[i].samples * clips[i].channels];//サンプルとチャネルで音源を生成
		// 		musicSource_.clip.GetData(waveData, Sections[i].StartTimeSamples);
		// 		clips[i].SetData(waveData, 0);//サンプルデータをセット
		...
		// 		sectionSource.clip = clips[i];//clipでーたを入れてる
		...
		OnSectionChanged();//削除するとupdateTimingが呼ばれなくなる。おそらくaddしたリストに移動してる
	}

	public void DebugX(){
		// Debug.Log("カレントセクション"+CurrentSection);
	}
	...
	public void OnValidate()//検証関数。セクションに値を代入して開始時間をずらしている
	{
		// Debug.Log("<color=red>Onvalidateが実行</color>");
		// Debug.Break;
		if( musicSource_ == null )
		{
			// Debug.Log("musicscoreがnull");
			//awakeと終了時によばれてる
			musicSource_ = GetComponent<AudioSource>();
		}
		if( samplingRate_ == 0 )
		{
			// Debug.Log("samplingrateが0");
			//awakeと終了時に呼ばれてる
			samplingRate_ = (musicSource_ != null && musicSource_.clip != null ? musicSource_.clip.frequency : 44100);//オーディオクリップのサンプリングレートが無ければ41000を代入
		}

		// if( Sections == null || Sections.Count == 0 )//オリジナル
		if( Sections == null || Sections.Count != 0 )//初手は1だから!=にしたら動いた
		{
			//starセクションがあるからcountは1なんだけど、セクション自体はデフォルトのままなので、この処理で上書きを行う

			// Debug.Log("Sectionがnullもしくは、Sections.contが0ではない");
		...
		{
			// Debug.Log("sectionは塗るじゃない今は"+Sections.Count);
			...
			// 		Debug.Log(i+"が上書きされてる？");//実行じには実行されない
			// 	}
	...
	{
		// Debug.Log("<color=red>OnDestroyが実行</color>");
		MusicList_.Remove(this);
		// Debug.Log("MusicList_の数は:"+MusicList_.Count);
	}

	void Initialize()
	{//再生時のjustとかを初期化している
		// Debug.Log("<color=red>Initializeが実行</color>");
	...
	//メモCurrentsectonは456付近にあるよ。ちなみにSectionのインデックスのゲッターだった
	//561のOnvalidateで呼ばれてる？
	public void OnSectionChanged()
	{
		// Debug.Log("<color=red>OnSectionChangedが実行</color>");
		if( Sections == null || Sections.Count == 0 ){
			Debug.Log("OnSectionChangedは実行されたが、そのままreturn");
			return;
		}
		if( CurrentSection_.Tempo > 0.0f )
		{
			// Debug.Log("CurrentSection_は:"+CurrentSection_);
			// Debug.Log("CurrentSection_.Tempoは:"+CurrentSection_.Tempo);
		...
		{
			// Debug.Log(CurrentSection_.Tempo+"が> 0.0f じゃなかったらしい");
			samplesPerUnit_ = 0;
			samplesPerBeat_ = 0;
			samplesPerBar_ = 0;//862行付近のバグにつながってる
			musicTimeUnit_ = 0;
		}
	}
	...
	{
		// Debug.Log("<color=red>PlayStarat関数が実行</color>");
	...
	{
		// Debug.Log("<color=red>SeekToSection関数が実行</color>");
	...
	{
		// Debug.Log("<color=red>SetNextSection関数が呼ばれました</color>");
	...
	{
		// Debug.Log("<color=red>ChecKClipChangeが実行</color>");
	...
	void UpdateTiming()
	{//おそらくデバッグテキストで見れる値を更新してくれてる
		// Debug.Log("<color=red>UpdateTiming関数が呼ばれました</color>");
		// find section index
		...
		else
		{
			...
			{
				if( Sections[sectionIndex_ + 1].StartTimeSamples <= currentSample_ )//スタートサンプルが今の時間を超えたら始動する
				{
		...
		if( sectionSample >= 0 )
		{
			just_.Bar = (int)(sectionSample / samplesPerBar_) + CurrentSection_.StartBar;//ここがエラー!!!!!!!!!!!!!!!!!!!!!!!!!!!691くらいに繋がる
	...
	void CallEvents()
	{
		// Debug.Log("<color=red>CallEventsが実行</color>");
	...
	void OnJustChanged()
	{
		// Debug.Log("<color=red>OnJustChangedが実行</color>");
...
public class Timing : IComparable<Timing>, IEquatable<Timing>
{
	...
	//自作
	public bool IsNull(){
		if(Bar==0 && Beat==0 && Unit==-1)	return true;
		else return false;
	} 
}